...
export default function AISearchBar({ onClear }: { onClear: () => void }) {
  ...
  async function doSearch(q: string) {
    ...
    try {
      ...
      const res = await apiClient.post<SearchResponse>('/api/menu/search', payload)
      ...
      <div style={{ padding: '24px' }}>
        ...
        <form onSubmit={handleSubmit} className="mb-5">
          <div className="flex gap-2">
            ...
            <button
              ...
              onMouseEnter={(e) => {
                if (!loading && query.trim()) e.currentTarget.style.backgroundColor = accentHover;
              }}
              onMouseLeave={(e) => {
                if (!loading && query.trim()) e.currentTarget.style.backgroundColor = accent;
              }}
            >
        ...
        {!results && !loading && (
          <div className="flex flex-wrap gap-2 mb-2">
            <span style={{ fontSize: '0.8rem', color: textMuted, alignSelf: 'center', marginRight: '2px', fontFamily: 'DM Sans, sans-serif' }}>
              Try saying:
            </span>
        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-2">
            {[1, 2, 3].map((i) => (
              ...
              >
                <div style={{ height: '14px', backgroundColor: '#F0EBE5', borderRadius: '6px', width: '75%', marginBottom: '12px' }} />
                <div style={{ height: '12px', backgroundColor: '#F5F0EB', borderRadius: '6px', width: '100%', marginBottom: '8px' }} />
                <div style={{ height: '12px', backgroundColor: '#F5F0EB', borderRadius: '6px', width: '66%', marginBottom: '16px' }} />
                <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between' }}>
                  <div style={{ height: '24px', backgroundColor: '#F0EBE5', borderRadius: '6px', width: '64px' }} />
                  <div style={{ height: '36px', backgroundColor: '#F0EBE5', borderRadius: '8px', width: '100px' }} />
                </div>
              </div>
            ))}
          </div>
        ...
        {error && (
          ...
          >
            <p style={{ color: accent, marginBottom: '8px', fontWeight: 500 }}>{error}</p>
            <button
              onClick={handleClear}
              style={{
                color: accent,
          ...
          <div className="mt-2">
            ...
            {results.items.length === 0 ? (
              <div className="text-center" style={{ padding: '48px 16px' }}>
                ...
                <p style={{ color: textBody, fontSize: '1.1rem', fontWeight: 500, marginBottom: '4px' }}>
              ...
              <>
                ...
                {results.relaxed_filters && results.items.length > 0 && results.items.length < 5 && (
                  <div
                    style={{
                      marginTop: '24px',
                      padding: '18px',
                  ...
                  >
                    <p style={{ color: accent, fontWeight: 500, marginBottom: '4px', fontFamily: 'DM Sans, sans-serif' }}>
                      Only {results.items.length} match{results.items.length !== 1 ? 'es' : ''} found
                    </p>
                    <p style={{ color: textBody, fontSize: '0.85rem', marginBottom: '12px', fontFamily: 'DM Sans, sans-serif' }}>
                      Filters were relaxed but results are still limited. Try a different search.
                    </p>
                    <button
                      onClick={handleClear}
                      style={{
