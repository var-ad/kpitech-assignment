...
export default function MenuManagement() {
  ...
  const [specialIds, setSpecialIds] = useState<Record<number, true>>({});
  ...
  const handleToggleAvailability = async (item: MenuItemData) => {
    ...
    try {
      await apiClient.patch(`/api/admin/menu-items/${item.id}/availability`, {
        available: !item.available,
      });
      fetchItems();
      toast(
  ...
  const handleToggleSpecial = async (item: MenuItemData) => {
    setSpecialIds((prev) => ({ ...prev, [item.id]: true }));
    try {
      await apiClient.patch(`/api/admin/menu-items/${item.id}/special`, {
        is_special: !item.is_special,
      });
      fetchItems();
      toast(
        item.is_special ? "Removed from specials" : "Marked as special",
        "success",
      );
    } catch (err: unknown) {
      const msg = getApiError(err, "Failed to update special status");
      toast(msg, "error");
    } finally {
      setSpecialIds((prev) => {
        const next = { ...prev };
  ...
  const handleDelete = (item: MenuItemData) => {
    if (
      !window.confirm(
        `Delete "${item.name}"? This item will be hidden from customers but preserved in order history.`,
      )
    )
      return;
    setDeletingIds((prev) => ({ ...prev, [item.id]: true }));
    apiClient
      .delete(`/api/admin/menu-items/${item.id}`)
      .then(() => {
        fetchItems();
  ...
  const filteredItems = normalizedSearch
    ? items.filter((item) => {
        const searchable = [
          ...
          item.description || "",
          item.category,
          String(parsePrice(item.price)),
          item.is_vegetarian ? "veg vegetarian" : "non-veg non vegetarian",
          item.is_spicy ? "spicy" : "not spicy",
  ...
  if (error) {
    return (
      <div style={{ textAlign: "center", padding: "40px 0" }}>
        <p style={{ color: accent, fontSize: "0.9rem", marginBottom: "8px" }}>
          {error}
        </p>
        <button
          onClick={fetchItems}
      ...
      >
        ...
        >
          ...
          {searchTerm && (
            <>
              {" "}
              · {filteredItems.length} matching "{searchTerm}"
            </>
          )}
        </span>
      ...
      {items.length === 0 ? (
        ...
        >
          ...
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "1.7fr 0.7fr 0.5fr 0.9fr 1.1fr 1fr",
              backgroundColor: headerBg,
              borderBottom: "1.5px solid",
              borderColor,
            }}
          >
            {["Name", "Category", "Price", "Tags", "Status", "Actions"].map(
              (h) => (
                <div
                  key={h}
                  style={{
          ...
          {filteredItems.map((item, idx) => {
            ...
            const isSpecialLoading = specialIds[item.id];
            return (
              <div
                key={item.id}
                style={{
                  display: "grid",
                  gridTemplateColumns: "2fr 0.8fr 0.6fr 1fr 0.7fr 1.2fr",
                  borderBottom:
                ...
                >
                  <span
                    style={{
                      ...
                      padding: "2px 8px",
                      borderRadius: "4px",
                      backgroundColor: item.is_vegetarian
                        ? "#E8F5E9"
                        : "#FFEBEE",
                ...
                <div style={cellStyle}>
                  <span
                    style={{
                      ...
                      color: item.available ? "#1A7A44" : "#C62828",
                    }}
                  >
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "4px",
                      border: "1.5px solid",
                      borderColor: item.available ? "#FFCDD2" : "#C8E6C9",
                      backgroundColor: "transparent",
                      color: item.available ? "#C62828" : "#1A7A44",
                      fontSize: "0.75rem",
                      fontWeight: 500,
                      cursor:
                        isToggling || isDeleting ? "not-allowed" : "pointer",
                      fontFamily: "DM Sans, sans-serif",
                      transition: "all 0.1s",
                      opacity: isToggling || isDeleting ? 0.5 : 1,
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.backgroundColor = item.available
                        ? "#FFEBEE"
                        : "#E8F5E9";
                    }}
                    onMouseLeave={(e) => {
                  ...
                  <button
                    ...
                    disabled={isSpecialLoading || isDeleting}
                    style={{
                      padding: "6px 12px",
                      borderRadius: "4px",
                      border: "1.5px solid",
                      borderColor: item.is_special ? "#FDE68A" : "#F5F0EB",
                      backgroundColor: item.is_special
                        ? "#FFFBEB"
                        : "transparent",
                      color: item.is_special ? "#D97706" : "#B0A89E",
                      fontSize: "0.75rem",
                      fontWeight: 500,
                      cursor:
                        isSpecialLoading || isDeleting
                          ? "not-allowed"
                          : "pointer",
                      fontFamily: "DM Sans, sans-serif",
                      transition: "all 0.1s",
                      opacity: isSpecialLoading || isDeleting ? 0.5 : 1,
                    }}
                    onMouseEnter={(e) => {
                      if (!item.is_special && !isSpecialLoading) {
                        e.currentTarget.style.borderColor = accent;
                        e.currentTarget.style.color = accent;
                      }
                    }}
                    onMouseLeave={(e) => {
                      if (!item.is_special && !isSpecialLoading) {
                        ...
                        e.currentTarget.style.color = "#B0A89E";
                      }
                    }}
                  >
                    {isSpecialLoading
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "6px",
                      border: "none",
                      backgroundColor: formSaving ? textMuted : accent,
                      color: "#FFFFFF",
                      fontSize: "0.8125rem",
                      fontWeight: 600,
                      cursor: formSaving ? "not-allowed" : "pointer",
